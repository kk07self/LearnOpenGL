...
//        ourShader.setVec3("light.position", lightPos); // 光源的位置
        ourShader.setVec3("light.position", camera.Position);   // 聚光的位置向量
        ourShader.setVec3("light.direction", camera.Front);     // 聚光的方向向量
        ourShader.setFloat("light.cutOff", glm::cos(glm::radians(12.5f))); // 切光角的余弦值（内锥形余弦值）
        ourShader.setFloat("light.outerCutOff", glm::cos(glm::radians(17.5f))); // 外锥形余弦值
